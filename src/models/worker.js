...
module.exports = {
  ...
  create: function(worker) {
    return db.getClient()
      .query('INSERT INTO worker (name, email, password, tags, sub_merchant_json) VALUES ($1, $2, $3, $4, $5) RETURNING *',[worker.name, worker.email, worker.password, worker.tags, worker.subMerchantJson])
      .then(function(results) {
        return results.rows[0]
      });
  ...
  validateLogin: function(email, password) {
    return db.getClient()
      .query('SELECT FROM worker WHERE email = $1' [email])
      .then(function(results) {
        if (results.length === 0) {
          throw 'No such email';
